...
def userinput(choosetype, ask, askhelp):
	mod = str.lower(input(ask)).strip()
	while choosetype(mod):
		mod = str.lower(input(askhelp)).strip()
	return mod
...
unfound = True
while unfound:
	filename = input('Enter the name of your blendfile (extension not necessary): ').strip()
	while filename == "help" or not filename:
		filename = input('Ex : "myfile.blend" or "myfile" if your in the same directory as the script.\nelse add the path : "subdir/myfile" or "subdir/myfile.blend" (without quotes)\nWrite your filename : ').strip()
...
if rendermode == "1":
	##one frame only
	#choose frame
	framenum = input('Frame number : ').strip()
	while not framenum or checknum(framenum):
		framenum = input('Please enter the number of the frame to render : ').strip()
	...
	if animmode == "y":
		...
		frameend = input('End frame : ').strip()
		while checknum(frameend) or int(frameend) < int(framestart):
			frameend = input('Set the final frame of your animation (it must be a equal or higher number than start): ').strip()
...
if outputmode == "y":
	...
	output = input('Write the path and output name according to the help above : ').strip()
	while not output:
		output = input('Write your output name (ex : "img_" or "render/imgs_####" wihtout quotes): ').strip()
